...
if (fs.existsSync(adminFile)) {
  ...
  content = content.replace(/<div>\s*<label[^>]*>\s*SEO Title<\/label>\s*<input[^>]*value=\{blog\.seo_title(?: \|\| '')?\}[^>]*>\s*<\/div>/g, '');
  content = content.replace(/<div>\s*<label[^>]*>\s*SEO Description<\/label>\s*<textarea[^>]*value=\{blog\.seo_description(?: \|\| '')?\}[^>]*><\/textarea>\s*<\/div>/g, '');
